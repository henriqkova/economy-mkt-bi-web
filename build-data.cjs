...
// ---------- filtro de anos para ALL_TX (reduz bundle pra funcionar em mobile) ----------
const TX_YEAR_MIN = REF_YEAR;
const TX_YEAR_MAX = REF_YEAR;
const txYearFilter = (t) => {
  if (!t.data_efetiva) return false;
  const y = t.data_efetiva.getFullYear();
  return y >= TX_YEAR_MIN && y <= TX_YEAR_MAX;
};
const txRecFiltered = recNorm.filter(t => txYearFilter(t) && (t.regime || 'caixa') === 'caixa');
const txDespFiltered = despNorm.filter(t => txYearFilter(t) && (t.regime || 'caixa') === 'caixa');
console.log(`  ALL_TX: filtrado anos ${TX_YEAR_MIN}-${TX_YEAR_MAX} + regime caixa: ${txRecFiltered.length + txDespFiltered.length} de ${recNorm.length + despNorm.length} rows (${((1 - (txRecFiltered.length + txDespFiltered.length) / (recNorm.length + despNorm.length)) * 100).toFixed(1)}% reduzido)`);
...
// ALL_TX: dictionary-encoded para reduzir bundle (mobile-friendly).
// Strings repetidas (categoria, cliente, fornecedor, centroCusto) sao substituidas
// por indices numericos no array _TX_D (dicionario). Decodificacao feita em runtime
// logo abaixo via _decodeTx().
// Schema row: [kind, mes, dia, catIdx, cliIdx, valor, realizado, fornIdx, ccIdx, regime, tags]
${(() => {
  const allFiltered = [
    ...txRecFiltered.map(t => ['r', t.data_efetiva ? t.data_efetiva.toISOString().slice(0,7) : '', t.data_efetiva ? t.data_efetiva.getDate() : 0, t.categoria, t.cliente, t.valor, t.realizado ? 1 : 0, '', t.centroCusto || '', (t.regime || 'caixa') === 'caixa' ? 'c' : 'k', t.tags || '']),
    ...txDespFiltered.map(t => ['d', t.data_efetiva ? t.data_efetiva.toISOString().slice(0,7) : '', t.data_efetiva ? t.data_efetiva.getDate() : 0, t.categoria, '', t.valor, t.realizado ? 1 : 0, t.cliente, t.centroCusto || '', (t.regime || 'caixa') === 'caixa' ? 'c' : 'k', t.tags || '']),
  ];
  // Build dictionary from fields 3 (cat), 4 (cli), 7 (forn), 8 (cc)
  const dictMap = new Map();
  let nextId = 0;
  const getIdx = (s) => { if (!s) return -1; if (dictMap.has(s)) return dictMap.get(s); dictMap.set(s, nextId); return nextId++; };
  const encoded = allFiltered.map(r => [r[0], r[1], r[2], getIdx(r[3]), getIdx(r[4]), r[5], r[6], getIdx(r[7]), getIdx(r[8]), r[9], r[10]]);
  const dictArr = new Array(dictMap.size);
  for (const [k, v] of dictMap) dictArr[v] = k;
  console.log(`  ALL_TX dict: ${dictMap.size} strings unicas, ${encoded.length} rows`);
  return `var _TX_D = ${JSON.stringify(dictArr)};
var _TX_RAW = ${JSON.stringify(encoded)};
var ALL_TX = _TX_RAW.map(function(r) { return [r[0],r[1],r[2],r[3]>=0?_TX_D[r[3]]:'',r[4]>=0?_TX_D[r[4]]:'',r[5],r[6],r[7]>=0?_TX_D[r[7]]:'',r[8]>=0?_TX_D[r[8]]:'',r[9],r[10]]; });
_TX_RAW = null;`;
})()}
