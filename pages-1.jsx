...
const PageOverview = ({ filters, setFilters, onOpenFilters, statusFilter, drilldown, setDrilldown, year, month }) => {
  ...
  // Receita de parcelas (filtrada do ALL_TX)
  const receitaParcelas = useMemo(() => {
    const rg = (filters && filters.regime === "competencia") ? "k" : "c";
    const sf = statusFilter || "realizado";
    ...
    var total = 0;
    for (var i = 0; i < txs.length; i++) {
      if (txs[i][0] === "r" && /parcelas?/i.test(txs[i][3])) total += txs[i][5];
    }
    return total;
  }, [statusFilter, filters, drilldown, year, refYear]);

  const vlParcelas = receitaParcelas - B.TOTAL_DESPESA;
  const indicadores = [
    { value: receitaParcelas,  label: "Receita de parcelas",  kind: "receita" },
    { value: B.TOTAL_DESPESA, label: "Soma de despesa",     kind: "despesa" },
    { value: vlParcelas,      label: "Valor líquido",       kind: vlParcelas >= 0 ? "receita" : "despesa" },
