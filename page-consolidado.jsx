...
const PageConsolidado = ({ filters, setFilters, statusFilter, drilldown, setDrilldown, year, month }) => {
  ...
  // Receita de parcelas (apenas "RECEITAS DE PARCELAS")
  const recParcelas = useMemo(() => {
    const rg = (filters && filters.regime === "competencia") ? "k" : "c";
    const sf = statusFilter || "realizado";
    const refY = year || refYear;
    var txs = window.filterTx ? window.filterTx(window.ALL_TX || [], sf, drilldown, rg, filters) : [];
    if (!drilldown) txs = txs.filter(r => r[1] && r[1].startsWith(String(refY)));
    var total = 0;
    var porMes = Array(12).fill(0);
    for (var i = 0; i < txs.length; i++) {
      if (txs[i][0] === "r" && /parcelas?/i.test(txs[i][3])) {
        total += txs[i][5];
        var mi = parseInt((txs[i][1] || "").slice(5, 7), 10) - 1;
        if (mi >= 0 && mi < 12) porMes[mi] += txs[i][5];
      }
    }
    return { total, porMes };
  }, [statusFilter, filters, drilldown, year, refYear]);

  const receita = recParcelas.total;
  ...
  return (
    <div className="page">
      ...
      <div className="row row-4">
        <KpiTile label="Receita de Parcelas" value={fmtK(receita)} tone="green" sparkValues={recParcelas.porMes} sparkColor="var(--green)" />
        ...
        <KpiTile label="Cancelamentos" value={fmtNC(canc.count)} tone="red" nonMonetary />
      </div>
